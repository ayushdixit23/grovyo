...
export default function LibraryLayout({ children }) {
  ...
  const navLinks = [
    {
      ...
      icon: <CiShoppingCart className="text-[20px]" />,
      label: "Cart",
      active: path === "/main/library" || path === "/main/library/Cart",
    },
    {
      href: "../../main/library/Track",
      icon: <CiShoppingBasket className="text-[20px]" />,
      label: "Track Order",
      active: path.startsWith("/main/library/Track"),
    },
    {
      href: "../../main/library/Subscription",
      icon: <MdOutlineSubscriptions className="text-[20px]" />,
      label: "Subscription",
      active: path.startsWith("/main/library/Subscription"),
    },
  ];
  const renderLink = ({ href, icon, label, active }) => (
    <Link
      href={href}
      className={`font-medium gap-1 sm:gap-2 flex justify-center items-center py-2 rounded-2xl text-[10px] sm:text-[12px] px-3 pp:px-4 ${
        active
          ? "bg-[#0075FF] text-white"
          : "text-[#3e3e3e] bg-[#f9f9f9] dark:bg-[#323d4e] dark:text-[#fff]"
      }`}
    >
      {icon}
      <div>{label}</div>
  ...
  return (
    ...
    //   </div>
    // </div >
    <div className="flex flex-row bg-white dark:bg-bluedark h-full w-full">
    <div className="h-full select-none flex flex-col items-start w-full md:border-r-2 border-[#f7f7f7]">
      <div className="md:h-[15vh] py-2 flex flex-col gap-1 sm:gap-0 border-b-2 bg-white border-r-2 w-full dark:bg-[#0D0D0D] dark:border-[#1d1d1d] dark:text-[#fff] pn:max-md:w-full pn:max-md:fixed top-14 md:min-w-[390px]">
        <div className="h-[50%] w-full flex flex-col text-[20px] font-bold px-4 py-2 justify-end">
          Library
        </div>
        <div className="h-[50%] w-full flex justify-start items-center text-black px-4 gap-2">
          {navLinks.map(renderLink)}
        </div>
      </div>
      <div className="h-[89vh] w-full">{children}</div>
    </div>
  </div>
  );
}
