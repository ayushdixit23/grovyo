...
export default function NewforyouLayout({ children }) {
	...
	const loadmoreData = async () => {
		try {
			const res = await axios.post(`${API}/post/fetchmore/${data?.id}`, { fetchAds: { adIds: [] } });
			if (res.data.success) {
	...
	return (
		<>
			...
			<div className="w-[100%] h-[100vh] bg-white dark:bg-[#0D0D0D] flex flex-col sm:flex-row pn:max-md:justify-center ">
				<div className={`${id && isMobile ? "hidden" : " pn:max-md:h-[96vh] lg:w-[27%] md:w-[32%] sm:w-[37%] h-screen md:overflow-auto scrollbar-hide select-none dark:border:[#273142] flex flex-col w-full items-center md:border-r-2 border-[#f7f7f7] dark:border-[#131619] self-end "}`}>
					{/* post 1*/}
				...
				{id && <div className="lg:w-[73%] md:w-[68%] sm:w-[63%] "><Newforyou id={id} /></div>}
				{!id && <div className="lg:w-[73%] md:w-[68%] sm:w-[63%]">{children}</div>}
			</div >
		</>
	);
}
